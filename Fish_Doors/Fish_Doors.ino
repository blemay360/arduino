...
Servo door1, door2, partition;
int doorSpeed = 2;
int doorDir = -1; //change to -1 if direction is backwards

int door1pos = 0, door2pos = 0, partitionPos = 0; //door position, defaults to down
...
void loop() {
	if (Serial.available() > 0) {
		...
		else if (signalIn == 51) { //press 3 to toggle partition
			togglePartition();
		}
		else if (signalIn == 52) {  //press 4 to toggle both doors
			toggleBothDoors();
		}
...
void openPartition() {
	Serial.println("Opening partition");
	for (int i = 0; i <= 180; i++) {
		partition.write(i);
		delay(15);
	}
...
void closePartition() {
	Serial.println("Closing partition");
	for (int i = 180; i > 0; i--) {
		partition.write(i);
		delay(15);
	}
